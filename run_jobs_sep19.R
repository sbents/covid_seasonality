...
# DATA ###################################################################################
#Location = c( "CT", "GA", "MI",  "MN", "NY",  "OH", "TN")
...
census_dat = data.frame(Location, pop_size_2023) # %>%
 # mutate(total = sum(pop_size_2023), fraction = pop_size_2023/total) %>%
 # mutate(adjust = (1/7)/fraction, check = adjust*fraction)
...
full_calibration_dat = read.csv("processed_calibration_dat.csv")  %>%
 # filter(Location %in% c( "CT", "MN", "OH", "MI", "NY", "TN", "GA" )) %>%
  ...
  mutate(inverse_temp = ifelse(inverse_temp > 26, inverse_temp*1.25, inverse_temp))
...
results <- run_hierarchical_mcmc_calibration(
  ...
  iterations = 1200,     # start small for testing
...
for(i in state){
 ...
 # print(head(tab_state))
  ...
  # states 
  N <- print(unique(dat$pop_size_2023.x))
 ...
 # S0 =  results$summary$statistics[8] * N
  ...
  param <- c(
    ...
    hum =  .36, # results$summary$statistics[2], 
   # hum = .0025, 
    temp =  0.05,  # results$summary$statistics[3],  
...
#    )
  ...
  out_df_check <- as.data.frame(out_check) %>%
    mutate(predicted_hosp = I* results$summary$statistics[6]+ .47*I2* results$summary$statistics[7] )
  
  print(out_df_check$predicted_hosp)
  
  # plot 
  plot =  ggplot() +
    geom_line(data = out_df_check %>% filter(time > 12), aes(x = time + 1 , y = predicted_hosp), col = "red",lwd = 2) +
...
combined_samples <- do.call(rbind, results$samples)

# Optionally add a column indicating the chain number
chain_ids <- rep(1:length(results$samples), each = nrow(results$samples[[1]]))
combined_samples_df <- as.data.frame(combined_samples)
combined_samples_df$chain <- chain_ids

# Save to CSV
write.csv(combined_samples_df, file = "mcmc_samples_output_sep17.csv", row.names = FALSE)
