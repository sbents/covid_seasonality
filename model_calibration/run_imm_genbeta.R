############################################################################################
# MODEL: Immunity with general beta 
  ...
  run_chain <- function(chain_id, iterations, burn_in, thin) {
    ...
    if(!is.finite(current_log_post)) {
      ...
      while(!is.finite(current_log_post) && attempts < 10) {
        current_params <- current_params * runif(length(current_params), 0.9, 1.10)
        current_log_post <- log_posterior(current_params)
        attempts <- attempts + 1
      }
...
# Save to CSV
write.csv(all_chains_df, "mcmc_chains_imm_genbeta_jan15.csv", row.names = FALSE)


sink("mcmc_summary_imm_genbeta_jan15.txt")
