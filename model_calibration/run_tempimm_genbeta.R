############################################################################################
# MODEL: Temperature and immunity with general beta 
...
#    }
 ...
 # state_initial <- list()
#  for(state_name in states) {
#    state_initial[[paste0("beta_", state_name)]] <- runif(1, 0.2, .65)
#  }
...
results <- run_hierarchical_mcmc_calibration(
  ...
  iterations = 500,     # start small for testing
...
# Save to CSV
write.csv(all_chains_df, "mcmc_chains_immtemp_genbeta_jan15.csv", row.names = FALSE)


### Post-processing MCMC results 

sink("mcmc_summary_immtemp_genbeta_jan15.txt")
