...
############################################################################################
# MODEL: Humidity and immunity with general beta 
...
run_hierarchical_mcmc_calibration <- function(full_calibration_dat, iterations = 18000, burn_in = 8000, thin = 2, n_chains = 3) {
  ...
  seirs_model_hybrid <- function(t, state, parms) {
    ...
    seasonal <- beta * (hum_scale*(humidity_at_t  - 40)^2 + hum)  
  ...
  run_chain <- function(chain_id, iterations, burn_in, thin) {
    ...
    # Fixed diagonal covariance matrix using 4% of parameter values
    # this means: 
    
    #  Takes each parameter's current value
    #  Multiplies by 0.04 to get the standard deviation for proposals
    # Example: if a parameter = 0.5, then proposal std = 0.5 × 0.04 = 0.02
    
    fixed_scales <- abs(current_params) * 0.04  
    ...
    for(i in 1:iterations) {
      ...
      if(i %% 500 == 0) {
        ...
        cat(sprintf("Chain %d, Iteration %d: Acceptance rate = %.3f (FIXED proposals)\n", 
  ...
  # run multiple chains
  cat("Starting MCMC,", n_chains, "chains,", iterations, "iterations\n")
...
# Save to CSV
write.csv(all_chains_df, "mcmc_chains_immhum_genbeta_feb23.csv", row.names = FALSE)

sink("mcmc_summary_immhum_genbeta_feb23.txt")
