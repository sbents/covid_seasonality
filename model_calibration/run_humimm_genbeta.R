...
run_hierarchical_mcmc_calibration <- function(full_calibration_dat, iterations = 18000, burn_in = 8000, thin = 2, n_chains = 3) {
  ...
  seirs_model_hybrid <- function(t, state, parms) {
    ...
    # seasonal function
    seasonal <- beta * ( hum* humidity_at_t )
...
results <- run_hierarchical_mcmc_calibration(
  ...
  iterations = 1800,     # start small for testing
  burn_in = 700,         # burn-in period
...
# Save to CSV
write.csv(all_chains_df, "mcmc_chains_immhum_genbeta_jan15.csv", row.names = FALSE)



sink("mcmc_summary_immhum_genbeta_jan15.txt")
