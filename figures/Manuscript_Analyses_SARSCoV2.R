...
ggplot(data = variant_dataset %>% filter(Location != "CT" &
                                           Location != "MD" &
                                           Location != "NM")) +
...
# Calibrate each state to improve fit from global step, using Latin Hypercube sampling
n <- 400 # number of samples
k <- 4     # number of parameters
U <- randomLHS(n, k)

head(params_df)
sus_frac <- qunif(U[,1], min = 0.67, max = .73)
beta <- qunif(U[,2], min = 0.37, max = 0.43)
h1 <- qunif(U[,3], min = 0.0001, max = 0.019)
h2 <- qunif(U[,4], min = 0.0008, max = 0.0060)

# states, transform humidity here 
LHS_dat = full_calibration_dat %>%
  filter(Location == "CO") %>%
  filter(t > 13) %>%
  mutate(t = row_number()) %>%
  mutate(immunity_scalar_fitted = ifelse(slope_pos > 0, 1.25, 1))

N <- unique(LHS_dat$pop_size_2023)
...
for(i in seq_len(n)){
  ...
  seirs_model_hybrid_state = function(t, state, param) {
    with(as.list(state), {
      ...
      t_idx <- round(t)
      if(t_idx < 1) t_idx <- 1
      if(t_idx > nrow(LHS_dat)) t_idx <- nrow(LHS_dat)
      
      vax_rate_at_t = LHS_dat$incident[t_idx] 
      humidity_at_t = LHS_dat$humid_smooth[t_idx] 
      temp_at_t = LHS_dat$inverse_temp[t_idx]
      # act_at_t <- LHS_dat$mean_indoor[t_idx]
      
      imm_vec = imm  * LHS_dat$immunity_scalar_fitted[t_idx]
      
      
      # Seasonal function 
      #   seasonal = beta*( hum*humidity_at_t + temp*temp_at_t  + act*act_at_t ) 
...
# Prepare to store performance assessment 
...
# Plot 10-best performing simulations 
performance_top10 = performance[1:10,]
...
for (k in seq_along(samples)) {
  ...
  seirs_model_hybrid_state = function(t, state, param) {
    with(as.list(state), {
      ...
      vax_rate_at_t = LHS_dat$incident[t] 
      humidity_at_t = LHS_dat$humid_smooth[t] 
      temp_at_t = LHS_dat$inverse_temp[t]
      #   act_at_t = LHS_dat$mean_indoor[t]
      
      imm_vec = imm  * LHS_dat$immunity_scalar_fitted[t]
  ...
  plot_top = ggplot(data = out_df_check_state) +
    geom_line(aes(x = date, y = rollmean(predicted_hosp, k = 2, na.pad = TRUE)/ (unique(LHS_dat$pop_size_2023)) *100000), col = "darkslategray3", lwd = 1.4) +
    geom_line(aes(x = date, y = rollmean(hosp, k = 2, na.pad = TRUE)/(unique(LHS_dat$pop_size_2023)) *100000), col = "black", lwd = 1.4,  alpha = 1) +
...
#####################################################################
# MAIN SIMULATION LOOP WITH UNCERTAINTY
#####################################################################
...
for(m in 1:length(states)) {
  ...
  for (sim in 1:n_simulations) {
    ...
    fits_sim <- fits %>%
      #  mutate(humid_smooth = ifelse(humid_smooth < 40, 
      #                              humid_smooth * sampled_hum_scale, 
      #                             humid_smooth)) %>%
      mutate(date = ISOweek2date(
        paste0(year, "-W", sprintf("%02d", week), "-1"))) %>%
...
for(m in 1:length(states)) {
  ...
  for (sim in 1:n_simulations) {
   ...
   # sampled_beta_scale <- rnorm(1, mean = best_beta[m], sd = param_sds$beta)
   # sampled_beta_scale <- max(sampled_beta_scale, 0)  # Ensure non-negative
    
    sampled_h1_scale <- rnorm(1, mean = best_h1[m], sd = param_sds$h1)
    sampled_h1_scale <- max(sampled_h1_scale, 0)  # Ensure non-negative
    
    sampled_h2_scale <- rnorm(1, mean = best_h2[m], sd = param_sds$h2)
    sampled_h2_scale <- max(sampled_h2_scale, 0)  # Ensure non-negative
    
  #  sampled_S0_scale <- rnorm(1, mean = best_S0[m], sd = param_sds$S0)
  #  sampled_S0_scale <- max(sampled_S0_scale, 0)  # Ensure non-negative
    ...
    # Initial conditions
    S0 = best_S0[m] * N #sampled_S0_scale*N # best_S0[m] * N
    E0 <- 0.01 * N
    I0 <- 0.001 * N
    R0 <- .94*N - S0 
    V0 <- 0.049 * N
    ...
    param <- c(
      ...
      imm = sampled_imm, 
      imm_vax = sampled_imm_vax,
      hr =  sampled_h1_scale , #best_h1[m],
      hr2 = sampled_h2_scale, #best_h2[m],
      beta = best_beta[m] , #sampled_beta_scale, #  best_beta[m],               # sampled_beta_scale,
      lp = 1/(4/7),
      gamma = 1/(4/7),              
      VE = .30,
      N = N
    ...
    out_df_sim <- as.data.frame(out_sim) %>%
     # mutate(predicted_hosp = I*best_h1[m] + I2*best_h2[m]) %>%
      mutate(predicted_hosp = I*sampled_h1_scale + I2*sampled_h2_scale) %>%
      mutate(t = time) %>%
      left_join(fits_sim, by = "t") %>%
      left_join(dat_time, by = c("t", "date", "week", "year")) %>%
      mutate(check_real = (hosp/pop_size_2023)*100000) %>%
  ...
  plot_best_fits[[m]] <- ggplot(data = summary_stats) +
    ...
    geom_ribbon(aes(x = date, 
                    ymin = rollmean(lower_95, k = 3, na.pad = TRUE), 
                    ymax = rollmean(upper_95, k = 3, na.pad = TRUE)), 
                fill = "darkslategray3", alpha = 0.75) +
    # 50% uncertainty ribbon (darker)
    geom_ribbon(aes(x = date, 
                    ymin = rollmean(lower_50, k = 3, na.pad = TRUE), 
                    ymax = rollmean(upper_50, k = 3, na.pad = TRUE)), 
...
r_effective = calibrated_states_SNL_df %>%
  ...
  #  filter(date > "2023-07-15" & date < "2024-03-01")
...
fig2d_final = plot_grid(plot_ex_2022_final, plot_ex_2023_final, ncol = 2 , rel_widths = c(.27, .3))
fig2d_final
...
topof2 = plot_grid(avg_imp, fig2d_final, rel_widths = c(.26, .6))
topof2
...
#plot_grid(fig2abc, fig2d_final, ncol = 1, rel_heights = c(.52, .26))
...
#####################################################################
# Figure 3 
# Plot time between peaks 2022-23
#diff_between_peaks = read.csv("processed_calibration_dat.csv")  %>%
...
#seas2223 = diff_between_peaks %>%
#  filter(time_period %in% c("Summer-22", "Winter-22/23")) %>%
#  dplyr::select(Location, Phenotype, time_period, week_peak_hosp) %>%
#  pivot_wider(
#    names_from = time_period,
#    values_from = week_peak_hosp
#  ) %>%
#  mutate( winter_summer_diff = `Winter-22/23` - `Summer-22` )
#head(seas2223)

#seas2324 = diff_between_peaks %>%
...
#    values_from = week_peak_hosp
#  ) %>%
#  mutate(`Winter-23/24` = ifelse(`Winter-23/24` < 10, `Winter-23/24` + 52, `Winter-23/24`)) %>%
#  mutate( winter_summer_diff= `Winter-23/24` - `Summer-23` ) 
#head(seas2324 )

# put them together 
#time_between_peaks = rbind(seas2223 %>% dplyr::select(Location, Phenotype, winter_summer_diff), 
#                           seas2324 %>% dplyr::select(Location, Phenotype, winter_summer_diff)) %>%
#  group_by(Phenotype) %>%
#  mutate(mean_diff = mean(winter_summer_diff)) %>% 
#  ungroup()
#pd <- position_dodge(width = 0.9)  # controls spacing between bars

#timing_diff_plot = ggplot(
#  data = time_between_peaks,
#  aes(x = reorder(Location, -mean_diff), y = mean_diff, fill = Phenotype)) +
#  geom_col(width = 0.7, position = pd) +   # narrower bars = visible gaps
#  geom_text(
#    aes(label = Location),
#    position = pd,
#    vjust = -0.3,        # puts text just above bar
#    size = 3 ) +
#  scale_fill_manual(
#    values = c(
#      "black","slateblue4","darkcyan", "aquamarine2" )) +
#  xlab("Time period") +
#  ylab("Mean time between winter and summer peak") +
#  theme_bw() +
#  theme(
...
#    axis.text.y     = element_text(size = 12), 
#    legend.position = "none")    # y-axis tick labels 
...
ggplot(data = all_sims_imm_df) +
  geom_line(aes(x = time, y = predicted_hosp, col = sim_id, group = sim_id))
...
ca_imm_new = ggplot() + 
  # Plot all 100 simulations colored by immunity duration
  geom_line(data = all_sims_imm_df %>% left_join(dat_time_state, by = "t"), # %>% filter(t > 12)
            ...
            alpha = 0.90, lwd = 1.5) +
  # Plot original simulation in black (imm = 0.05, duration = 20 weeks)
...
#row_tn <- plot_grid(tn_vax1, tn_imm_new, nrow = 1)
#row_tn
...
plot_grid(timing_peak_plot, final_fig3, ncol = 1 , labels = c("A", "B") )
...
#left_fig4 = plot_grid(hosp_vax_timing_plot,  auc_comparison, ncol = 1, rel_heights = c(.7, .2))
...
for(j in 1:length(states)) {
  ...
  for(i in 1:length(lag_values)){
    ...
    peak_row <- dat_simulated_vax_final %>%
      ...
      slice(1)   # In case of ties, take first occurrence
...
pt_vaxsitch1 = ggplot(data = peak_timing) +
  ...
  # scale_y_continuous(
  #   breaks = seq(-1, 7, 2), labels = seq(-1, 7, 2)) +
  theme_bw() +
  theme(legend.position = "bottom") +
  theme(
    plot.title      = element_text(size = 16, face = "bold"),   # title
...
#ggplot(data = full_calibration_dat %>% 
#         filter(time > 2023.5) %>%
#         group_by(Location) %>%
#         mutate(cum = cumsum(incident))) +
#  geom_line(aes(x = time, y = cum)) +
#  facet_wrap(vars(Location))
# Simulate vaccination curves 
